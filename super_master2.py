...
from master import pick_prompt
from master import update_prompt_result
...
def create_chatgpt_account(sb):
    ...
    with sync_playwright() as p:
        ...
        for i in range(1000):
            for sel in LIST_OF_POPUPS: 
                ...
                if cdp.is_element_visible(sel):
                    ...
                    break
            picked_prompt=pick_prompt(website_filter={"url": "https://sequence.xyz/", "location": "US"})
            print(f"entering prompt: {picked_prompt}")
        ...
        if _all_results:
            ...
            print(f"[PW] Total captured: {len(_all_results)}/{len(i)} prompts")
        else:
